...
(When "^I run shell \"\\([^\"]*\\)\"$"
  (lambda (command)
    (let* ((buffer-name "*ecukes shell output*")
           (buffer
            (progn
           ...
           (exit-code
            (apply
             'call-process-shell-command command nil (list buffer t))))
...
(When "^I run hg \"\\([^\"]*\\)\"$"
  (lambda (command)
    (let* ((buffer-name "*ecukes hg output*")
           (buffer
            (progn
              (when (get-buffer buffer-name)
                (kill-buffer buffer-name))
...
(When "^I start with a fresh repository$"
  (lambda ()
    ...
    (let* ((default-directory (file-name-as-directory monky-repo-path)))
      (When "I run hg \"init\"")
      (copy-file (f-join monky-features-path "hgrc") (f-join ".hg" "hgrc"))
